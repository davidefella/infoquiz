...
class EvaluationControllerTest {
    ...
    @Test
    void getEvaluationDetails_ShouldReturnEvaluationDetailsDTO() throws Exception {
        MvcResult authResult = mvc.perform(post(ApiEndpoints.AUTH_TOKEN)
                        .with(httpBasic("T_fd@gmail.com", "password")))
                .andExpect(status().isOk())
                .andReturn();
        ...
        mvc.perform(get(ApiEndpoints.TEACHER_EVALUATIONS_V1 + "/" + UUIDRegistry.EVALUATION_1) .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.evaluationUuid").value(Matchers.equalToIgnoringCase(UUIDRegistry.EVALUATION_1.toString())));
    }

    @Test
    void getEvaluationDetails_ShouldReturnNotFound_WhenEvaluationNotFound() throws Exception {
        MvcResult authResult = mvc.perform(post(ApiEndpoints.AUTH_TOKEN)
                        .with(httpBasic("T_fd@gmail.com", "password")))
                .andExpect(status().isOk())
                .andReturn();
        ...
        mvc.perform(get(ApiEndpoints.TEACHER_SESSIONS_V1 + "/22222222-2222-2222-2222-222222222222").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk());
